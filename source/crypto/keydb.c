...
u32 LoadKeyDb(const char* path_db, AesKeyInfo* keydb, u32 bsize) {
    ...
    if (path_db) {
        ...
        if (f_open(&fp, INPUT_PATH "/" KEYDB_NAME, FA_READ | FA_OPEN_EXISTING) == FR_OK) {
...
u32 LoadKeyFromFile(void* key, u32 keyslot, char type, char* id)
{
    ...
    AesKeyInfo* keydb = (AesKeyInfo*) WORK_BUFFER;
    u32 nkeys = LoadKeyDb(NULL, keydb, WORK_BUFFER_SIZE);
    ...
    if (!found && (type != 'I')) {
        ...
        FIL fp;
        UINT btr;
        snprintf(path, 64, "%s/slot0x%02lXKey%s%s.bin", INPUT_PATH, keyslot,
...
u32 InitKeyDb( void )
{
    ...
    // try to load aeskeydb.bin file
    AesKeyInfo* keydb = (AesKeyInfo*) (void*) WORK_BUFFER;
    u32 nkeys = LoadKeyDb(NULL, keydb, WORK_BUFFER_SIZE);
    if (!nkeys) return 1;
